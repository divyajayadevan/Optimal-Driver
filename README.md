...
│       ├── web_exporter.py                   # Interactive HTML dashboard generator
│       └── templates/dashboard.html          # Dashboard page template
├── web/
│   └── index.html                            # Interactive dashboard (generated by main.py)
├── tests/
...
Add `--open-web` to open the interactive dashboard (`web/index.html`) in your browser when the run finishes.
...
4. **Interactive Web Dashboard** ([`web/index.html`](web/index.html)):
   - A single self-contained page (open it directly, no server needed; charts load Plotly from a CDN).
   - **Hungarian steps**: every step of the real 21×21 run (row reduction, column reduction, line covering, θ-adjustments, final matching) as an annotated matrix you can step through.
   - **Algorithm sandbox**: run the same algorithm on your own small matrix, step by step, checked against brute force.
   - Interactive cost-matrix heatmap with per-cell cost breakdown, solver benchmark, bipartite allocation network, workload and utilization charts, sortable data tables, and a replay of the terminal output.
5. **MATLAB Matrix & Solver** ([`matlab_assignment_solver.m`](matlab_assignment_solver.m) & [`data/processed/optimal_driver_data.mat`](data/processed/optimal_driver_data.mat)):
